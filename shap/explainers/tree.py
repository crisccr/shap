    ...
    def shap_values(self, X, y=None, tree_limit=None, approximate=False, check_additivity=True):
        ...
        if self.model.model_output == "log_loss":
            ...
            assert X.shape[0] == len(
                y), "The number of labels (%d) does not match the number of samples to explain (%d)!" % (len(y), X.shape[0])
...
class TreeEnsemble:
    ...
    def __init__(self, model, data=None, data_missing=None, model_output=None):
        ...
        # does the background dataset land in every leaf (making it valid for the tree_path_dependent method)
        ...
        # If this is greater than 1 it means we have multiple stacked models with the same number of trees in each model (XGBoost multi-output style)
        self.num_stacked_models = 1
        ...
        if type(model) is dict and "trees" in model:
            ...
            self.trees = [Tree(e.tree_, scaling=scaling, data=data, data_missing=data_missing)
            ...
            scaling = 1.0 / len(model.estimators_)  # output is average of trees
            self.trees = [IsoTree(e.tree_, scaling=scaling, data=data, data_missing=data_missing)
                          for e in model.estimators_]
            self.tree_output = "raw_value"
            ...
            self.input_dtype = np.float32
            scaling = 1.0 / len(model.estimators_)  # output is average of trees
            self.trees = [Tree(e.tree_, normalize=True, scaling=scaling, data=data,
                               data_missing=data_missing) for e in model.estimators_]
            self.objective = objective_name_map.get(model.criterion, None)
            self.tree_output = "probability"
        # TODO: add unit test for this case
        elif safe_isinstance(model, ["sklearn.ensemble.ExtraTreesClassifier", "sklearn.ensemble.forest.ExtraTreesClassifier"]):
            assert hasattr(model, "estimators_"), "Model has no `estimators_`! Have you called `model.fit`?"
            self.internal_dtype = model.estimators_[0].tree_.value.dtype.type
            self.input_dtype = np.float32
            scaling = 1.0 / len(model.estimators_)  # output is average of trees
            ...
            self.trees = [Tree(e.tree_, scaling=model.learning_rate, data=data, data_missing=data_missing)
            ...
            if safe_isinstance(model.init_, ["sklearn.ensemble.LogOddsEstimator", "sklearn.ensemble.gradient_boosting.LogOddsEstimator"]):
                ...
                self.base_offset = scipy.special.logit(model.init_.class_prior_[1])
                self.tree_output = "log_odds"
            else:
                assert False, "Unsupported init model type: " + str(type(model.init_))
            ...
            if safe_isinstance(model, "pyspark.ml.classification.RandomForestClassificationModel") \
                ...
                self.trees = [Tree(tree, normalize=normalize, scaling=model.treeWeights[i]/sum_weight)
            ...
            # Spark GBT, create 1 weighted (learning rate) tree per sub-model
                ...
                self.tree_output = "raw_value"
                self.trees = [Tree(tree, normalize=False, scaling=model.treeWeights[i])
                              for i, tree in enumerate(model.trees)]
            ...
            assert safe_isinstance(model.base_models[0][param_idx], [
                                   "sklearn.tree.DecisionTreeRegressor", "sklearn.tree.tree.DecisionTreeRegressor"]), "You must use default_tree_learner!"
            ...
            self.trees = [Tree(e.tree_, scaling=s, data=data, data_missing=data_missing)
                          for e, s in zip(shap_trees, scaling)]
        ...
        if self.trees is not None and self.trees:
            ...
            assert len(np.unique([t.values.shape[1] for t in self.trees])
                       ) == 1, "All trees in the ensemble must have the same output dimension!"
    ...
    def get_transform(self):
        ...
        if self.model_output == "raw":
            ...
            if self.tree_output == "log_odds":
                ...
                raise SHAPError(
                    "model_output = \"probability\" is not yet supported when model.tree_output = \"" + self.tree_output + "\"!")
            ...
            if self.objective == "squared_error":
                ...
                raise SHAPError(
                    "model_output = \"log_loss\" is not yet supported when model.objective = \"" + self.objective + "\"!")
        else:
            raise SHAPError("Unrecognized model_output parameter value: %s! If model.%s is a valid function open a github issue to ask that this method be supported." % (
                str(self.model_output), str(self.model_output)))
        ...
        # deprecated dictionary support (with sklearn singlular style "feature" and "value" names)
            ...
            def buildTree(index, node):
                ...
                if tree._java_obj.getImpurity() == 'variance':
                    ...
                    self.values[index] = [e for e in node.impurityStats().stats()]
                # weighted count of element trough this node
                self.node_sample_weight[index] = node.impurityStats().count()

                if node.subtreeDepth() == 0:
                    return index
                else:
                    # index of the feature we split on, not available for leaf, int
                    self.features[index] = node.split().featureIndex()
                    ...
                    self.thresholds[index] = node.split().threshold()
    ...
    def get_trees(self, data=None, data_missing=None):
        ...
        for tree_index in range(self.num_trees):
            ...
            for index in range(len(leaf_weights) - 2, 0, -1):
                leaf_weights_unraveled[index] = leaf_weights_unraveled[2 *
                                                                       index + 1] + leaf_weights_unraveled[2 * index + 2]
